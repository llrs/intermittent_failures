...
intermittent_failures[lengths(dependencies) == 0] <- TRUE
```
...
These is only on dependencies on CRAN not outside like Bioconductor or other additional repositories (There are `r sum(!is.na(ap[, "Additional_repositories"]))` packages with additional repositories). 

If they have some dependencies and those dependencies didn't change as far as we can tell then they might be some problems with random numbers or connectivity.

```{r intermittent-failures}
...
We finally show the differences on the status (.t from today .y from yesterday):

```{r table-failures}
filter(possible_packages, Package %in% packages) |> 
  select(-Version.t, -Version.y) |> 
  flextable() |> 
  autofit()
```
